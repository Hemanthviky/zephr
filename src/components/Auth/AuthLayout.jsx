 ...
 * suppressed until there's room. From `lg` it splits: the pitch takes the left
 * half at full size and the card sits beside it, because a 420px column
 * marooned in the middle of a desktop window is the single clearest sign that
 * nobody thought about this screen.
 */
...
export default function AuthLayout({ mode, onSwitch, children }) {
  return (
    <div className="relative min-h-[100dvh] overflow-hidden px-5 pb-safe pt-safe">
      {/* Ambient 3D props. Suppressed on phones, where they'd crowd the form. */}
      <Icon3D
        name="avocado"
        size={110}
        float
        className="pointer-events-none absolute -left-6 top-24 hidden opacity-90 sm:block lg:bottom-14 lg:left-6 lg:top-auto"
      />
      <Icon3D
        name="curry"
        size={96}
        float
        className="pointer-events-none absolute -right-4 top-56 hidden opacity-90 sm:block lg:hidden"
      />

      <div className="mx-auto flex min-h-[100dvh] w-full max-w-[1120px] flex-col lg:grid lg:grid-cols-2 lg:items-center lg:gap-16">
        {/* ── Pitch ──────────────────────────────────────────────────────── */}
        <header className="mx-auto w-full max-w-[420px] pt-10 text-center lg:mx-0 lg:max-w-none lg:pt-0 lg:text-left">
          ...
          <h1 className="font-display text-[2.6rem] font-extrabold leading-[0.92] tracking-tighter lg:text-[4rem]">
            Eat it. Spend it.
            <br />
            <span className="relative inline-block">
              <span className="relative z-10">Know it.</span>
              <span className="absolute inset-x-[-6px] bottom-1 z-0 h-4 -rotate-1 rounded bg-lime-300 lg:bottom-2 lg:h-6" />
            </span>
          </h1>

          <p className="mx-auto mt-4 max-w-[19rem] text-[0.95rem] font-medium leading-snug text-ink-500 lg:mx-0 lg:mt-6 lg:max-w-[26rem] lg:text-lg">
            Two trackers, one login. You type what happened; Zephr does the
            arithmetic and remembers it.
          </p>

          {/* The module breakdown is desktop-only — on a phone it would push
              the form itself below the fold, which is the whole point of the page. */}
        ...
        {/* ── Form ───────────────────────────────────────────────────────── */}
        <main className="mx-auto mt-8 w-full max-w-[420px] flex-1 lg:mx-0 lg:mt-0 lg:flex-none lg:justify-self-end">
